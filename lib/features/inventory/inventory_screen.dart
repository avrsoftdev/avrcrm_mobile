...
class _InventoryScreenState extends State<InventoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: isDark ? scheme.surface : const Color(0xFFF5F8FC),
      appBar: AppBar(
        elevation: 0,
        titleSpacing: 24,
        title: const Row(
      ...
      body: StreamBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          final products = docs.where((data) {
          ...
          for (final data in docs) {
            final quantity = (data['quantity'] ?? 0) is num
          ...
          final categories = <String>{
            'All',
            ...docs.map((data) => (data['category'] ?? 'Other').toString()),
          };
            ...
            children: [
              ...
              Expanded(
                child: products.isEmpty
                    ? _EmptyInventory(onAdd: () => _openProductForm())
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final status = _calculateStockStatus(data);
                          final stockColor = _stockColor(status, scheme);
                          ...
                          return Card(
                            ...
                            child: InkWell(
                              ...
                              onTap: () => _openProductForm(product: data),
                              child: Padding(
...
class _ProductFormDialogState extends State<ProductFormDialog> {
  ...
  void initState() {
    ...
    if (data != null) {
      _productName.text = data['productName']?.toString() ?? '';
      _sku.text = data['sku']?.toString() ?? '';
      _barcode.text = data['barcode']?.toString() ?? '';
      _description.text = data['description']?.toString() ?? '';
      _category.text = data['category']?.toString() ?? '';
      _brand.text = data['brand']?.toString() ?? '';
      _hsn.text = data['hsn']?.toString() ?? '';

      _quantity.text = data['quantity']?.toString() ?? '0';
      _unit.text = data['unit']?.toString() ?? 'Nos';
      _reorderLevel.text = data['reorderLevel']?.toString() ?? '0';

      _purchasePrice.text = data['purchasePrice']?.toString() ?? '';
      _sellingPrice.text = data['sellingPrice']?.toString() ?? '';
      _mrp.text = data['mrp']?.toString() ?? '';

      _gst.text = data['gst']?.toString() ?? '18';

      _warehouse.text = data['warehouse']?.toString() ?? '';
      _rack.text = data['rack']?.toString() ?? '';
  ...
  Future<void> _save() async {
    ...
    final data = {
      ...
      'gst': double.tryParse(_gst.text) ?? 0,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _grid([
                        ...
                        TextFormField(
                          controller: _hsn,
                          decoration: _decoration('HSN / SAC Code'),
                        ),
                      ]),
                      ...
                      TextFormField(
                        ...
                        maxLines: 3,
                        decoration: _decoration('Product Description'),
                      ),
                      ...
                      _grid([
                        TextFormField(
                          ...
                          decoration: _decoration('Current Quantity'),
                        ),
                        TextFormField(
                          controller: _unit,
                        ...
                        TextFormField(
                          controller: _rack,
                          decoration: _decoration('Rack'),
                        ),
                        TextFormField(
                          controller: _bin,
                          decoration: _decoration('Bin'),
                        ),
                      ]),
                      const SizedBox(height: 28),
                      _sectionTitle(
                      ...
                      _grid([
                        ...
                        TextFormField(
                          controller: _mrp,
                          keyboardType: TextInputType.number,
                          decoration: _decoration('MRP'),
                        ),
                        TextFormField(
                          controller: _gst,
                          keyboardType: TextInputType.number,
                          decoration: _decoration('GST %'),
                        ),
                      ]),
                      const SizedBox(height: 28),
                      _sectionTitle(
                      ...
                      TextFormField(
                        ...
                        decoration: _decoration('Supplier / Vendor'),
                      ),
                    ],
                  ),
                ),
